...
|---|---|---|---|
| OD-1 | How to register in the shared `installed.json` | **`--skills-dir` for all iterative work (P0–P4). A single, backed-up, atomic edit to `installed.json` only at the start of the P5 integration run, with a documented revert.** See P0 for the exact entry + backup/atomic/validate procedure (OPS-1). | Need for hook/sessionStart features (which `--skills-dir` cannot load) before P5. |
